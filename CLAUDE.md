...
- ASI05: OS command probe activo (`child_process.execSync('id')` para JS, `subprocess.check_output(['id'])` para Python). Output confirmation para AutoGen y Langflow. Blind timing probe: warmup request previo + threshold 0.5s (evita cold-TCP FP de ~200ms en primer request). `_check_letta_tools_run()` — POST `/v1/tools/run` con Python exec payload sin auth (GHSA-p67m-xf4h-2r78); CRITICAL 98/90/80 según confirmación de output.
